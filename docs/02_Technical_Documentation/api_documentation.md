...
**POST** `/leads/bulk`
- **Purpose**: Bulk Upsert leads via JSON array.
- **Payload**: `[{ "first_name": "...", "email": "...", ... }]`
- **Logic**: Upsert based on `(email, location_id)`.

**POST** `/leads/bulk/csv`
- **Purpose**: Bulk Upsert leads via CSV File Upload.
- **Headers**: `Content-Type: multipart/form-data`
- **Body**: `file` (CSV File)
- **Format**: `first_name,last_name,email,mobile,notes,status,staff_name,staff_id,source`
- **Logic**: Parses CSV, maps headers (firstname, lastname, etc.), and executes bulk upsert.
