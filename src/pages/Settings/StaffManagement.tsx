        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
          ...
          {!staffToEdit && (
            <Typography variant="caption" color="text.secondary">
              * The new staff member will receive an email to activate their
    ...
    try {
      ...
      if (Array.isArray(response)) {
        const sorted = [...response].sort((a, b) => {
          ...
          return (
            (order[a.role as string] ?? 3) - (order[b.role as string] ?? 3)
          );
        });
        setStaffList(sorted);
                        ...
                        {staff.role?.toUpperCase() !== "SUPERADMIN" && (
                          <Tooltip title="Send Activation/Reset Link">
                            <IconButton
                              ...
                              onClick={() =>
                                handleSendResetLink(staff.staff_id)
                              }
                            >
                              {sendingEmail === staff.staff_id ? (
