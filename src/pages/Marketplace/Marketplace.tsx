...
export const Marketplace = () => {
  ...
  const accountId =
    paramAccountId || userParams?.account_id || userParams?.account?.account_id;
  const isMember = role === "MEMBER" || role === "USER";

  const [loading, setLoading] = useState(true);
  const [marketplaceError, setMarketplaceError] = useState<string | null>(null);
  const [basePrices, setBasePrices] = useState<BasePrice[]>([]);
  const [membershipPrograms, setMembershipPrograms] = useState<
    MembershipProgram[]
  >([]);
  ...
  const [manualMembershipOverride, setManualMembershipOverride] =
    useState(false);
  ...
  const [pendingBaseCard, setPendingBaseCard] = useState<BasePlanCard | null>(
    null,
  );
  const [selectedTermId, setSelectedTermId] = useState<string>("");
  const [selectedBaseStartDate, setSelectedBaseStartDate] =
    useState<string>("");
  ...
  const [pendingMembershipCandidate, setPendingMembershipCandidate] =
    useState<CategoryCandidate | null>(null);
  ...
  const [pendingServiceSelection, setPendingServiceSelection] =
    useState<ServicePackSelection | null>(null);

  // Waiver Integration State
  const [waiverTemplates, setWaiverTemplates] = useState<WaiverTemplate[]>([]);
  const [waiverPhase, setWaiverPhase] = useState<
    "BEFORE_PAYMENT" | "AFTER_INFO_CAPTURED" | "AFTER_PAYMENT" | null
  >(null);
  const [contractsDialogOpen, setContractsDialogOpen] = useState(false);
  const [previewDialogOpen, setPreviewDialogOpen] = useState(false);
  const [previewItem, setPreviewItem] = useState<CartItem | null>(null);
  const [checkoutWaiverCallback, setCheckoutWaiverCallback] = useState<
    (() => void) | null
  >(null);
  const [capturedLast4, setCapturedLast4] = useState<string>("");
  const [paymentInfoCapturedCallback, setPaymentInfoCapturedCallback] =
  ...
  const [membershipDiscounts, setMembershipDiscounts] = useState<
    Record<string, { value: string; explanation: string }>
  >({});

  // Only SUPERADMIN / ADMIN / STAFF may enter manual discounts
  const canApplyManualDiscount = ["SUPERADMIN", "ADMIN", "STAFF"].includes(
    role ?? "",
  );
  ...
  const [selectedMembershipAgeGroupIds, setSelectedMembershipAgeGroupIds] =
    useState<string[]>([]);
  ...
  const subscriptionTermNameMap = useMemo(() => {
    return new Map(
      subscriptionTerms.map((term) => [term.subscription_term_id, term.name]),
    );
  }, [subscriptionTerms]);

  const subscriptionTermMap = useMemo(() => {
    return new Map(
      subscriptionTerms.map((term) => [term.subscription_term_id, term]),
    );
  }, [subscriptionTerms]);
  ...
  const familyMembers = useMemo(
    () =>
      profiles.filter(
        (profile) =>
          primaryMember && profile.profile_id !== primaryMember.profile_id,
      ),
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!currentLocationId) {
        setMarketplaceError(
          "Please select a location before using Marketplace.",
        );
        setLoading(false);
        return;
      }
      if (!accountId) {
        setMarketplaceError(
          "Unable to resolve your account context. Please log out and log in again.",
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (currentLocationId && accountId) {
          try {
            ...
            const mappedCart: CartItem[] = items
              ...
              .map((i) => {
                ...
                return {
                  ...
                  type:
                    metadata.type ||
                    (i.subscription_type === "SERVICE"
                      ? "SERVICE"
                      : i.subscription_type === "LESSON_REGISTRATION_FEE"
                        ? "REGISTRATION_FEE"
                        : i.subscription_type === "MEMBERSHIP_FEE"
                          ? "BASE"
                          : "MEMBERSHIP"),
            ...
            const membershipItems = mappedCart.filter(
              (i) => i.type === "MEMBERSHIP",
            );
            const primaryBaseItems = mappedCart.filter(
              (i) =>
                i.type === "BASE" &&
                (i.metadata?.role === "PRIMARY" ||
                  i.name.includes("(PRIMARY)")),
            );
            ...
            if (membershipItems.length > 1) {
              ...
              const bestItem =
                membershipItems.find((i) => i.id === STABLE_MEMBERSHIP_ID) ||
                membershipItems[0];
              deduplicatedCart = deduplicatedCart.filter(
                (i) =>
                  i.type !== "MEMBERSHIP" || i.cart_id === bestItem.cart_id,
              );
            ...
            mappedCart.forEach((item) => {
              const hasRealDiscount =
                ...
                Math.abs(item.actualPrice - item.price) > 0.01;
              if (item.discountCode || hasRealDiscount) {
                initialDiscounts[item.id] = {
                  ...
                  manualAmount: item.discountCode
                    ? ""
                    : item.discountAmount?.toString() || "",
                  manualPercentage: item.discountCode
                    ? ""
                    : item.discountPercentage?.toString() || "",
                  membershipDiscountExplanation: (item.metadata as any)
                    ?.membershipExplanation,
                  autoDiscountDisabled: (item.metadata as any)
                    ?.autoDiscountDisabled,
                };
              }
            });
        ...
        try {
          const accountResponse = await crmService.getAccountDetails(
            accountId,
          ...
          const accountProfiles = [
            ...(accountData.profiles || accountData.profile || []),
          ].sort((a, b) => {
          ...
          setPrimaryProfileId(
            primary?.profile_id || accountProfiles[0]?.profile_id || null,
          );
        } catch {
        ...
        setWaiverTemplates(
          Array.isArray(waiversResponse)
            ? waiversResponse
            : (waiversResponse as any)?.data || [],
        );
        ...
        const enrichedServices = await Promise.all(
          rawServices.map(async (service) => {
            ...
            try {
              const packsResponse = await serviceCatalog.getServicePacks(
                serviceId,
                currentLocationId,
              );
              ...
              const packsWithPrices = await Promise.all(
                packs.map(async (pack) => {
                  ...
                  try {
                    const pricesResponse = await serviceCatalog.getPackPrices(
                      pack.service_pack_id,
                      currentLocationId,
                    );
                    const prices = Array.isArray(pricesResponse)
                      ? (pricesResponse as ServicePriceLike[])
                      : (pricesResponse as { data?: ServicePriceLike[] })
                          ?.data || [];
                    return { ...pack, prices };
                  } catch {
                    return { ...pack, prices: [] as ServicePriceLike[] };
        ...
        const allSectionIds = [
          "MEMBERSHIP",
          ...enrichedServices.map((s) => s.service_id),
        ];
  ...
  useEffect(() => {
          ...
          try {
            const ownerId =
              item.type === "MEMBERSHIP"
                ? item.membershipCategoryId!
                : item.referenceId;
            const services = await membershipService.getServices(
              ownerId,
              currentLocationId || undefined,
            );
            services.forEach((s) => {
            ...
            console.error(
              "Failed to fetch membership/base services for discount",
              error,
            );
          }
        }),
    ...
    JSON.stringify(
      cart
        .filter((item) => item.type === "MEMBERSHIP" || item.type === "BASE")
        .map((i) => i.id),
    ),
    currentLocationId,
  ]);
  ...
  useEffect(() => {
    ...
    setCart((prev) => {
      ...
      const next = prev.map((item) => {
        ...
        const discState = itemDiscounts[item.id] || {
          code: "",
          codeStatus: "idle",
          manualAmount: "",
          manualPercentage: "",
        ...
        if (item.discountCode && item.discountCode !== "MEMBERSHIP")
          return item;
        if (
          item.discountCode === "MEMBERSHIP" &&
          item.metadata?.membershipExplanation === disc.explanation
        )
          return item;

        const originalPrice = item.actualPrice ?? item.price;
        const { amount, percentage } = parseDiscountValue(
          disc.value,
          originalPrice,
        );
        ...
        const discountedPrice = Math.max(
          0,
          parseFloat((originalPrice - amount).toFixed(2)),
        );
    ...
    return Array.from(grouped.values())
      .map((card) => ({
        ...
        terms: [...card.terms].sort((left, right) => {
          const leftSort = getTermSortValue(
            subscriptionTermOrder,
            left.subscription_term_id,
          );
          const rightSort = getTermSortValue(
            subscriptionTermOrder,
            right.subscription_term_id,
          );
  ...
  const filteredBasePlanCards = useMemo(() => {
    ...
    return basePlanCards.filter((card) =>
      selectedMembershipAgeGroupIds.includes(card.ageGroupId),
    );
  }, [basePlanCards, selectedMembershipAgeGroupIds]);

  const baseCartItems = useMemo(
    () => cart.filter((item) => item.type === "BASE"),
    [cart],
  );
  ...
  const householdCounts = useMemo(
    () => buildHouseholdCountsFromBaseCart(baseCartItems, ageGroups),
    [baseCartItems, ageGroups],
  );
  const missingDobProfileIds = useMemo(
    () => getBaseCartMissingDobProfileIds(baseCartItems),
    [baseCartItems],
  );
  ...
  const suggestedCandidate = useMemo(() => {
    ...
    baseCartItems.length,
    eligibleCandidates,
    householdCounts,
    missingDobProfileIds.length,
    ageGroups,
  ]);
  ...
  useEffect(() => {
    const syncMembership = async () => {
      if (
        manualMembershipOverride ||
        !suggestedCandidate ||
        baseCartItems.length === 0 ||
        missingDobProfileIds.length > 0 ||
      ...
      try {
        ...
        const STABLE_MEMBERSHIP_ID = "MEMBERSHIP-PLAN-HOUSEHOLD";
        const existingMembershipItem = cart.find(
          (item) =>
            item.type === "MEMBERSHIP" || item.id === STABLE_MEMBERSHIP_ID,
        );
        ...
        if (!existingMembershipItem) {
          ...
          if (accountId && currentLocationId) {
            try {
              const dbPayload = cartService.transformToCartData(
                membershipItem,
                accountId,
                currentLocationId,
              );
              const saved = await cartService.upsertItem(
                dbPayload,
                currentLocationId,
              );
          ...
          const needsCategoryUpdate =
            existingMembershipItem.membershipCategoryId !==
            suggestedCandidate.categoryId;
          const needsCoverageUpdate =
            getCoverageSig(existingMembershipItem.coverage) !== aggregatedSig;

          if (needsCategoryUpdate || needsCoverageUpdate) {
            const feeType = existingMembershipItem.feeType || "JOINING";
            const newPrice =
              feeType === "JOINING"
                ? suggestedCandidate.joiningFee
                : suggestedCandidate.annualFee;
            const finalPrice = needsCategoryUpdate
              ? newPrice || 0
              : existingMembershipItem.price;
            ...
            if (accountId && currentLocationId) {
              try {
                ...
                if (updatedItem.cart_id)
                  dbPayload.cart_id = updatedItem.cart_id;
                const saved = await cartService.upsertItem(
                  dbPayload,
                  currentLocationId,
                );
                ...
                console.error(
                  "Failed to auto-update membership on server:",
                  err,
                );
              }
            }
            ...
            setCart((prev) => {
              const hasOtherMembership = prev.some(
                (item) =>
                  item.type === "MEMBERSHIP" &&
                  item.id !== STABLE_MEMBERSHIP_ID &&
                  item.id !== existingMembershipItem.id,
              );
  ...
  const isInCart = (idPrefix: string) =>
    cart.some(
      (item) => item.id === idPrefix || item.id.startsWith(`${idPrefix}-`),
    );
  ...
  const hasActivePrimary = useMemo(() => {
    ...
    const cartHasPrimary = cart.some((item) => {
      ...
      return (
        item.metadata?.role === "PRIMARY" || item.name.includes("(PRIMARY)")
      );
    });
    ...
    const subHasPrimary = subscriptions.some(
      (sub) =>
        sub.subscription_type.startsWith("MEMBERSHIP") &&
        sub.status === "ACTIVE" &&
        (sub.coverage?.some((c: any) => c.role === "PRIMARY") ||
          sub.subscription_coverage?.some((c: any) => c.role === "PRIMARY")),
    );
  ...
  const getItemDiscount = useCallback(
    (itemId: string): ItemDiscountState => {
      return (
  ...
  const parseDiscountValue = (
    ...
    const percentage =
      originalPrice > 0
        ? parseFloat(((amount / originalPrice) * 100).toFixed(2))
        : 0;
      ...
      setCart((prev) =>
        prev.map((item) => {
          ...
          if (updatedItem.cart_id && accountId && currentLocationId) {
            cartService
              .upsertItem(
                cartService.transformToCartData(
                  updatedItem,
                  accountId,
                  currentLocationId,
                ),
  ...
  const handleApplyDiscountCode = useCallback(
    async (itemId: string, serviceId?: string, skipAuth = false) => {
      ...
      if (!discount) {
        setItemDiscounts((prev) => ({
          ...
          [itemId]: {
            ...getItemDiscount(itemId),
            codeStatus: "invalid",
            codeError: "Discount code not found or invalid.",
          },
        }));
        return;
      }
      ...
      if (!discount.is_active) {
        setItemDiscounts((prev) => ({
          ...
          [itemId]: {
            ...getItemDiscount(itemId),
            codeStatus: "invalid",
            codeError: "This discount code is inactive.",
          },
        }));
        return;
      }
        ...
        if (today < start) {
          setItemDiscounts((prev) => ({
            ...
            [itemId]: {
              ...getItemDiscount(itemId),
              codeStatus: "invalid",
              codeError: "This discount code is not yet active.",
            },
          }));
          return;
        }
        ...
        if (today > end) {
          setItemDiscounts((prev) => ({
            ...
            [itemId]: {
              ...getItemDiscount(itemId),
              codeStatus: "invalid",
              codeError: "This discount code has expired.",
            },
          }));
          return;
        }
      ...
      const resolvedCategory =
        ...
        discount.applicable_refs?.[0]?.discount_category ??
        null;
      const applicableRefIds = (discount.applicable_refs || [])
        .filter(
          (ref) =>
            !resolvedCategory || ref.discount_category === resolvedCategory,
        )
        .map((ref) => ref.reference_id);
      const referenceIds =
        applicableRefIds.length > 0
          ? applicableRefIds
      ...
      if (resolvedCategory === "SERVICE") {
        ...
        if (cartItem.type !== "SERVICE") {
          setItemDiscounts((prev) => ({
            ...
            [itemId]: {
              ...getItemDiscount(itemId),
              codeStatus: "invalid",
              codeError: "This discount code is only applicable to services.",
            },
          }));
          return;
        }
        ...
        if (
          ...
          !referenceIds.includes(cartItem.serviceId ?? "")
        ) {
          setItemDiscounts((prev) => ({
            ...prev,
            [itemId]: {
              ...
              codeStatus: "invalid",
              codeError:
                "This discount code is not applicable to this service.",
            },
          }));
          return;
        }
        ...
        if (cartItem.type !== "BASE") {
          setItemDiscounts((prev) => ({
            ...
            [itemId]: {
              ...
              codeStatus: "invalid",
              codeError:
                "This discount code is only applicable to membership fees.",
            },
          }));
          return;
        }
        ...
        if (
          referenceIds.length > 0 &&
          !referenceIds.includes(cartItem.referenceId ?? "")
        ) {
          setItemDiscounts((prev) => ({
            ...prev,
            [itemId]: {
              ...
              codeStatus: "invalid",
              codeError:
                "This discount code is not applicable to this membership fee.",
            },
          }));
          return;
        }
        ...
        if (cartItem.type !== "MEMBERSHIP") {
          setItemDiscounts((prev) => ({
            ...
            [itemId]: {
              ...
              codeStatus: "invalid",
              codeError:
                "This discount code is only applicable to membership plans.",
            },
          }));
          return;
        }
        ...
        if (
          referenceIds.length > 0 &&
          !referenceIds.includes(cartItem.membershipCategoryId ?? "")
        ) {
          setItemDiscounts((prev) => ({
            ...prev,
            [itemId]: {
              ...
              codeStatus: "invalid",
              codeError:
                "This discount code is not applicable to this membership plan.",
            },
          }));
          return;
        }
  ...
  const handleManualDiscountApply = useCallback(
    (
      itemId: string,
      ...
      setCart((prev) =>
        prev.map((item) => {
          ...
          if (type === "amount") {
            ...
            amount = parseFloat(
              (originalPrice * (percentage / 100)).toFixed(2),
            );
          }
          const discountedPrice = Math.max(
            0,
    ...
    (itemId: string) => {
      ...
      setItemDiscounts((prev) => ({
        ...
        [itemId]: {
          ...
          manualAmount: "",
          manualPercentage: "",
          autoDiscountDisabled: isMembership
            ? true
            : prev[itemId]?.autoDiscountDisabled,
        },
      }));
    },
    [cart],
  ...
  const openCoverageDialogForItem = (item: CartItem) => {
    ...
    const ageCategory =
      item.type === "SERVICE" || item.type === "REGISTRATION_FEE"
        ? "Service"
        : "Membership";
    setPendingItem(item);

    const initialSelection =
      existing?.coverage?.map((profile) => profile.profile_id) ||
    ...
    const allowedSelection = initialSelection.filter((id) => {
      ...
      if (item.type === "BASE" && item.ageGroupId) {
        if (profile.date_of_birth) {
          const ageGroupName = getAgeGroupName(
            profile.date_of_birth,
            ageGroups,
            ageCategory,
          );
          const matchedGroup = ageGroups.find(
            (group) =>
              group.name === ageGroupName &&
              group.age_group_category === ageCategory,
          );
        ...
        if (profile.date_of_birth) {
          const ageGroupId = classifyAgeFromDob(
            profile.date_of_birth,
            ageGroups,
            ageCategory,
          );
      ...
      if (isAllowed && (item.type === "BASE" || item.type === "MEMBERSHIP")) {
        ...
        const profileHasActive = subscriptions.some(
          (sub) =>
            sub.subscription_type.startsWith("MEMBERSHIP") &&
            sub.status === "ACTIVE" &&
            (sub.coverage?.some((c: any) => c.profile_id === id) ||
              sub.subscription_coverage?.some((c: any) => c.profile_id === id)),
        );
  ...
  const handleConfirmCoverage = () => {
    ...
    const ageCategory =
      pendingItem.type === "SERVICE" || pendingItem.type === "REGISTRATION_FEE"
        ? "Service"
        : "Membership";
    ...
    if (accountId && currentLocationId) {
      Promise.all(
        itemsToProcess.map(async (nextItem) => {
          const frontendId = nextItem.id;
          const isPrimaryBase =
            nextItem.type === "BASE" &&
            (nextItem.metadata?.role === "PRIMARY" ||
              nextItem.name.includes("(PRIMARY)"));
          ...
          const existingItem = nextItem.cart_id
            ...
            : cart.find((i) => {
                ...
                if (nextItem.type === "MEMBERSHIP" && i.type === "MEMBERSHIP")
                  return true;
                if (
                  isPrimaryBase &&
                  i.type === "BASE" &&
          ...
          const savedItem = await cartService.upsertItem(
            dbPayload,
            currentLocationId,
          );
        ...
        .then((savedItems) => {
          setCart((prev) => {
            ...
            savedItems.forEach((savedItem) => {
              ...
              const isPrimaryBase =
                savedItem.type === "BASE" &&
                (savedItem.metadata?.role === "PRIMARY" ||
                  savedItem.name.includes("(PRIMARY)"));

              if (savedItem.type === "MEMBERSHIP" || isPrimaryBase) {
                const existingIndex = nextCart.findIndex((cartItem) => {
                  if (savedItem.type === "MEMBERSHIP")
                    return cartItem.type === "MEMBERSHIP";
                  return (
                    cartItem.type === "BASE" &&
                    (cartItem.metadata?.role === "PRIMARY" ||
                      cartItem.name.includes("(PRIMARY)"))
                  );
                });
                ...
                const existingIndex = nextCart.findIndex(
                  (cartItem) => cartItem.id === savedItem.id,
                );
                if (existingIndex >= 0) {
                  nextCart[existingIndex] = savedItem;
                } else {
  ...
  const handleStartAddBaseCard = (card: BasePlanCard) => {
    if (card.role === "ADD_ON" && !hasActivePrimary) {
      setMarketplaceError(
        "You cannot add an Add-on membership without a Primary membership. Please add a Primary plan to your cart first, or ensure you have an active Primary subscription.",
      );
  ...
  const handleConfirmBaseTermSelection = () => {
    ...
    const selectedTerm = pendingBaseCard.terms.find(
      (term) => term.subscription_term_id === selectedTermId,
    );
    ...
    if (selectedBaseStartDate && selectedTermId) {
      ...
      if (effectiveTerm) {
        ...
        const value =
          effectiveTerm.recurrence_unit_value ||
          effectiveTerm.duration_months ||
          1;
  ...
  const handleConfirmFeeSelection = () => {
    ...
    const membershipItem: CartItem = {
      ...
      metadata: {
        joiningFee: pendingMembershipCandidate.joiningFee,
        annualFee: pendingMembershipCandidate.annualFee,
        type: "MEMBERSHIP",
      },
    };
  ...
  const handleOpenServiceSelection = (
    service: ServiceWithPacks,
    pack: ServicePackWithPrices,
  ) => {
  ...
  const handleConfirmServiceSelection = (newItems: CartItem[]) => {
    ...
    newItems.forEach((item) => {
      ...
      cartService
        .upsertItem(dbPayload, currentLocationId!)
        .then((saved) => {
          // Update the item in local cart with the real cart_id?
          setCart((prev) =>
            prev.map((p) =>
              p.id === item.id ? { ...p, cart_id: saved.cart_id } : p,
            ),
          );
        })
        .catch(console.error);
  ...
  const getCoveragePayload = (item: CartItem) => {
    ...
    const hasPrimary = coverage.some(
      (member) => profilesById.get(member.profile_id)?.is_primary,
    );

    return coverage.map((member, index) => {
      const isPrimary = profilesById.get(member.profile_id)?.is_primary;
      const roleLabel: "PRIMARY" | "ADD_ON" =
        isPrimary || (!hasPrimary && index === 0) ? "PRIMARY" : "ADD_ON";
  ...
  const constructSubscriptionPayload = (item: CartItem) => {
    ...
    const topLevelRole = coverage.find((c) => c.role === "PRIMARY")
      ? "PRIMARY"
      : "ADD_ON";

    // Get session_id from item or metadata
    const sessionId =
      item.session_id || (item.metadata as any)?.session_id || null;
    ...
    const payload: Record<string, unknown> = {
      ...
      discount_amount:
        ...
        Math.abs(item.discountAmount) > 0.01
          ? item.discountAmount
          : 0,
      discount_percentage:
        item.discountPercentage !== undefined &&
        Math.abs(item.discountPercentage) > 0.1
  ...
  const handlePaymentInfoCaptured = (last4: string): Promise<boolean> => {
    return new Promise((resolve) => {
      const afterInfoTemplates = waiverTemplates.filter(
        (t) => t.is_after_payment_info_captured === true,
      );
      const requiresWaivers = cart.some((item) => {
        if (item.type === "SERVICE")
          return afterInfoTemplates.some(
            (t) => t.service_id === item.serviceId,
          );
          ...
          return afterInfoTemplates.some(
            (t) =>
              (item.subscriptionTermId
                ? t.template_category?.toLowerCase() === "membership" &&
                  t.subterm_id === item.subscriptionTermId
                : false) || t.base_price_id === item.referenceId,
          );
        return false;
      });
  ...
  const handleCreateInvoiceForCheckout = async (): Promise<{
    invoiceId: string;
    total: number;
  } | null> => {
    const staffName =
    ...
    try {
      ...
      if (createdSubIds.length > 0 && accountId && currentLocationId) {
        ...
        const invResponse = await billingService.createInvoice(
          invoicePayload,
          currentLocationId,
        );
        const newInvoiceId =
          invResponse?.data?.invoice_id || invResponse?.invoice_id;
        ...
        if (newInvoiceId) {
          ...
          await Promise.all(
            createdSubIds.map((subId: string) =>
              billingService.updateSubscriptionInvoice(
                subId,
                newInvoiceId!,
                currentLocationId,
              ),
          ...
          return { invoiceId: newInvoiceId, total: totalAmount };
        }
      }
      return null;
    } catch (err) {
      console.error("Failed to create or link invoice:", err);
  ...
  const processCheckout = async () => {
    ...
    try {
      ...
      const beforePaymentTemplates = waiverTemplates.filter(
        (t) => t.is_before_payment === true,
      );
      const requiresWaivers = cart.some((item) => {
        if (item.type === "SERVICE")
          return beforePaymentTemplates.some(
            (t) => t.service_id === item.serviceId,
          );
        if (item.type === "MEMBERSHIP")
          return beforePaymentTemplates.some(
      ...
      setMarketplaceError("Failed to prepare checkout. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        <Box>
          <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
            Primary Member:{" "}
            {primaryMember ? getProfileDisplayName(primaryMember) : "N/A"}
          </Typography>
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap">
          {Array.from(accountAgeGroupIds).map((ageGroupId) => {
            const ageGroupName =
              ageGroups.find((group) => group.age_group_id === ageGroupId)
                ?.name || "Unknown";
              ...
              <Stack direction="row" spacing={1.5} flexWrap="wrap">
                ...
                {services.map((service) => (
                  <Chip
                    ...
                    color={
                      activeSections.includes(service.service_id)
                        ? "primary"
                        : "default"
                    }
                    variant={
                ...
                <Box sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                      {ageGroups
                        .filter((ag) => accountAgeGroupIds.has(ag.age_group_id))
                        .map((group) => {
                          const isSelected =
                            selectedMembershipAgeGroupIds.includes(
                              group.age_group_id,
                            );
                          return (
                            <Chip
                      ...
                      {selectedMembershipAgeGroupIds.length > 0 && (
                        <Button
                          size="small"
                          onClick={() => setSelectedMembershipAgeGroupIds([])}
                        >
                          Clear
                        </Button>
                      )}
                    ...
                    <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
                      ...
                      <Grid container spacing={2}>
                        {filteredBasePlanCards.map((card) => {
                          const selectedTerms = card.terms.filter(
                            (term) =>
                              term.base_price_id &&
                              isInCart(`BASE-${term.base_price_id}`),
                          );
                              ...
                              >
                                <CardContent>
                                  ...
                                  <Stack spacing={1} sx={{ mb: 2 }}>
                                    {card.terms.map((term) => {
                                      const inCart = term.base_price_id
                                        ? isInCart(`BASE-${term.base_price_id}`)
                                        : false;
                                      const termName =
                                        subscriptionTermNameMap.get(
                                          term.subscription_term_id,
                                        ) ||
                                        term.term_name ||
                                        "Term";
                                      const termInfo = subscriptionTermMap.get(
                                        term.subscription_term_id,
                                      );
                                      const billingText =
                                        termInfo?.payment_mode ===
                                          "RECURRING" ||
                                        term.payment_mode === "RECURRING"
                                  ...
                                  >
                                    <Typography
                                      ...
                                      color="text.secondary"
                                    >
                                      Joining or renewal fees are selected when
                    ...
                    {false && (
                      ...
                      >
                        ...
                        <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
                          <Chip
                            label={`Children: ${householdCounts.children}`}
                            size="small"
                          />
                          <Chip
                            label={`Adults: ${householdCounts.adults}`}
                            size="small"
                          />
                          <Chip
                            label={`Seniors: ${householdCounts.seniors}`}
                            size="small"
                          />
                        </Stack>
                        {baseCartItems.length === 0 && (
                          <Typography color="text.secondary">
                            Add at least one base plan selection to generate
                        ...
                        {baseCartItems.length > 0 &&
                          missingDobProfileIds.length > 0 && (
                            <Alert severity="warning">
                              Suggestion is blocked because DOB is missing for:{" "}
                              {missingDobNames.join(", ")}.
                            </Alert>
                          )}
                        ...
                        {baseCartItems.length > 0 &&
                          ...
                          suggestedCandidate && (
                            <Paper
                              sx={{ p: 2, bgcolor: "#ede9fe", borderRadius: 2 }}
                            >
                              <Typography
                                variant="subtitle1"
                                sx={{ fontWeight: 700, mb: 0.5 }}
                              >
                                Suggested: {suggestedCandidate?.programName} -{" "}
                                {suggestedCandidate?.categoryName}
                              </Typography>
                              <Typography
                                variant="body2"
                                color="text.secondary"
                              ...
                              <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
                                <Chip
                                  label={
                                    typeof suggestedCandidate?.joiningFee ===
                                    "number"
                                ...
                                <Chip
                                  label={
                                    typeof suggestedCandidate?.annualFee ===
                                    "number"
                              ...
                              <Button
                                variant="contained"
                                onClick={() =>
                                  handleOpenFeeDialog(suggestedCandidate!)
                                }
                              >
                                Add Suggested Plan
                              </Button>
                        ...
                        {baseCartItems.length > 0 &&
                          ...
                          !suggestedCandidate && (
                            <Alert severity="info">
                              No eligible membership plan found for this
                              household. You can still add a plan manually.
                            </Alert>
                          )}
                      </Paper>
                    )}
                      ...
                      >
                        {services.map((service) => (
                          ...
                          >
                            <Checkbox
                              checked={
                                selectedServiceIds.indexOf(service.service_id) >
                                -1
                              }
                            />
                      ...
                      .map(({ service, pack }) => {
                        const minPrice =
                          (pack.prices || []).length > 0
                            ? Math.min(
                                ...(pack.prices || []).map((priceItem) =>
                                  parseNumericPrice(priceItem.price),
                                ),
                              )
                            : 0;

                        const serviceItemId = pack.service_pack_id
                          ? `SERVICE-${pack.service_pack_id}`
                          : "";
                        const selected = serviceItemId
                          ? isInCart(serviceItemId)
                          : false;
                              ...
                              <CardContent sx={{ flexGrow: 1, p: 3 }}>
                                ...
                                >
                                  <Box>
                                    ...
                                    {service.LessonRegistrationFee !==
                                      undefined &&
                                      service.LessonRegistrationFee > 0 && (
                                        <Typography
                                          variant="caption"
                                          sx={{
                                ...
                                >
                                  ...
                                  {pack.duration_months ? (
                                    <Chip
                                      label={`${pack.duration_months} MONTHS`}
                                      size="small"
                                      sx={{
                                        height: 18,
                                        fontSize: "0.6rem",
                                  ...
                                  >
                                    {(pack.prices || []).map(
                                      (priceItem, idx) => {
                                        ...
                                        const students =
                                          (priceItem as any).num_students || 1;
                                        const instructors =
                                          (priceItem as any).num_instructors ||
                                          1;
                                ...
                                >
                                  ...
                                  <Button
                                    ...
                                    variant="contained"
                                    onClick={() =>
                                      handleOpenServiceSelection(service, pack)
                                    }
                                  >
                                    Select
                                  </Button>
                      ...
                      >
                        ...
                        {item.type === "MEMBERSHIP" && (
                          <Box sx={{ mb: 1 }}>
                            <RadioGroup
                              ...
                              onChange={(e) => {
                                const newFeeType = e.target.value as
                                  | "JOINING"
                                  | "ANNUAL";
                        ...
                        >
                          ...
                          <Typography
                            variant="body2"
                            color={
                              hasDiscount ? "success.main" : "primary.main"
                            }
                            fontWeight={700}
                          >
                        ...
                        >
                          ...
                          {(() => {
                            const hasTpl =
                              item.type === "SERVICE"
                                ? waiverTemplates.some(
                                    (t) => t.service_id === item.serviceId,
                                  )
                                : item.type === "MEMBERSHIP"
                                  ? waiverTemplates.some(
                                      (t) =>
                                        t.membership_category_id ===
                                        item.membershipCategoryId,
                                    )
                                  : item.type === "BASE"
                                    ? waiverTemplates.some(
                                        (t) =>
                                          ...
                                          (item.subscriptionTermId
                                            ? t.template_category?.toLowerCase() ===
                                                "membership" &&
                                              t.subterm_id ===
                                                item.subscriptionTermId
                                            : false) ||
                            ...
                            <Box>
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  disabled={
                                    !discState.code.trim() ||
                                    discState.codeStatus === "validating"
                                  }
                                  sx={{
                                    whiteSpace: "nowrap",
                                    fontSize: "0.75rem",
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) =>
                                      setItemDiscounts((prev) => ({
                                        ...
                                        [item.id]: {
                                          ...getItemDiscount(item.id),
                                          manualPercentage: e.target.value,
                                          manualAmount: "",
                                        },
                                      }))
                                    }
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Select a term and start date for {pendingBaseCard?.name} (
            {pendingBaseCard?.ageGroupName}).
          </Typography>
          ...
          <FormControl fullWidth>
            ...
            >
              {(pendingBaseCard?.terms || []).map((term) => {
                const termName =
                  subscriptionTermNameMap.get(term.subscription_term_id) ||
                  term.term_name ||
                  "Term";
                const termInfo = subscriptionTermMap.get(
                  term.subscription_term_id,
                ...
                return (
                  <Paper
                    key={term.base_price_id || term.subscription_term_id}
                    variant="outlined"
                    sx={{ p: 1.5, mb: 1.5 }}
                  >
                    <FormControlLabel
        ...
        <DialogActions>
          ...
          <Button
            onClick={handleConfirmBaseTermSelection}
            variant="contained"
            disabled={!selectedTermId || !selectedBaseStartDate}
          >
            Continue
          </Button>
        </DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Choose which fee to apply for{" "}
            {pendingMembershipCandidate?.programName} -{" "}
            {pendingMembershipCandidate?.categoryName}.
          </Typography>
          <FormControl fullWidth>
            <RadioGroup
              value={selectedFeeType}
              onChange={(event) =>
                setSelectedFeeType(event.target.value as "JOINING" | "ANNUAL")
              }
            >
              <FormControlLabel
                value="JOINING"
                disabled={
                  typeof pendingMembershipCandidate?.joiningFee !== "number"
                }
                control={<Radio />}
                label={
                  <Typography variant="body2">
                    Joining:{" "}
                    {typeof pendingMembershipCandidate?.joiningFee === "number"
                      ? formatCurrency(pendingMembershipCandidate.joiningFee)
                      : "Not configured"}
                  </Typography>
                }
              />
              <FormControlLabel
                value="ANNUAL"
                disabled={
                  typeof pendingMembershipCandidate?.annualFee !== "number"
                }
                control={<Radio />}
                label={
                  <Typography variant="body2">
                    Renewal:{" "}
                    {typeof pendingMembershipCandidate?.annualFee === "number"
                      ? formatCurrency(pendingMembershipCandidate.annualFee)
                      : "Not configured"}
                  </Typography>
                }
              />
        ...
        <DialogContent>
          ...
          <FormGroup>
            {profiles.map((profile) => {
              ...
              const ageCategory =
                pendingItem?.type === "SERVICE" ||
                pendingItem?.type === "REGISTRATION_FEE"
                  ? "Service"
                  : "Membership";
              ...
              if (pendingItem?.type === "BASE" && pendingItem.ageGroupId) {
                ...
                pendingItem?.type === "MEMBERSHIP" &&
                pendingItem.membershipRange
              ) {
                if (profile.date_of_birth) {
                  const ageGroupId = classifyAgeFromDob(
                    profile.date_of_birth,
                  ...
                  if (maxAllowed === 0) {
                    isAllowed = false;
                    restrictionReason =
                      "This profile is not eligible for this membership plan";
                  }
                } else {
                  isAllowed = false;
                  restrictionReason = "Date of birth missing";
              ...
              if (
                ...
                const profileInCart = cart.some(
                  (item) =>
                    ...
                    item.coverage?.some(
                      (c) => c.profile_id === profile.profile_id,
                    ) &&
                ...
                if (profileInCart) {
                  ...
                  const profileHasActive = subscriptions.some(
                    (sub) =>
                      ...
                      sub.status === "ACTIVE" &&
                      (sub.coverage?.some(
                        (c: any) => c.profile_id === profile.profile_id,
                      ) ||
                        sub.subscription_coverage?.some(
                          (c: any) => c.profile_id === profile.profile_id,
                        )),
                  );
                  if (profileHasActive) {
              ...
              return (
                <Tooltip
                  ...
                  placement="right"
                  disableHoverListener={isAllowed}
                >
                  <FormControlLabel
                    sx={{
                      mb: 1,
                      ...
                      <Checkbox
                        ...
                        onChange={(event) => {
                          if (event.target.checked) {
                            ...
                            setSelectedProfileIds((prev) =>
                              prev.filter((id) => id !== profile.profile_id),
                            );
                          }
                        }}
                      />
                      ...
                      <Box display="flex" alignItems="center" gap={1}>
                        ...
                        {profile.is_primary && (
                          <StarIcon
                            sx={{
                              fontSize: 14,
                              color: isAllowed ? "#f59e0b" : "text.disabled",
                            }}
                          />
                        )}
                      </Box>
      ...
      <Dialog
        open={otherPlansOpen}
        onClose={() => setOtherPlansOpen(false)}
        maxWidth="md"
        fullWidth
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            All active categories are listed below. Eligible categories are
            marked based on current household selection.
          </Typography>
          <Stack spacing={1.5}>
            {eligibilityResults.map((entry) => (
              <Paper
                key={entry.candidate.categoryId}
                variant="outlined"
                sx={{
                  p: 2,
                  borderRadius: 2,
                  bgcolor: entry.eligible ? "#ecfeff" : "#f8fafc",
                  borderColor: entry.eligible ? "#a5f3fc" : "divider",
                }}
              >
                <Box
                  sx={{
                ...
                >
                  <Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
                      {entry.candidate.programName} -{" "}
                      {entry.candidate.categoryName}
                    </Typography>
                ...
                {eligibilityResults.map((entry) => (
                  <TableRow key={`info-${entry.candidate.categoryId}`}>
                    ...
                    <TableCell align="right">
                      {typeof entry.candidate.joiningFee === "number"
                        ? formatCurrency(entry.candidate.joiningFee)
                        : "n/a"}
                    </TableCell>
                    <TableCell align="right">
                      {typeof entry.candidate.annualFee === "number"
                        ? formatCurrency(entry.candidate.annualFee)
                        : "n/a"}
                    </TableCell>
                    <TableCell>
                      {ageGroups.map((group, idx) => {
                        const r = (entry.candidate.range as any)[
                          group.age_group_id
                        ];
                        const min = r?.min ?? 0;
                ...
                {eligibilityResults.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} align="center">
                      <Typography color="text.secondary">
                        No membership categories configured.
                      </Typography>
                    </TableCell>
                  </TableRow>
                )}
      ...
      <PaymentDialog
        ...
        onSkip={async () => {
          ...
          try {
            const result = await handleCreateInvoiceForCheckout();
            if (result) {
              if (accountId && currentLocationId) {
          ...
          navigate(
            isMember
              ? "/portal?tab=invoices"
              : `/admin/accounts/${accountId}?tab=invoices`,
          );
        }}
        onSuccess={async () => {
          const afterPaymentTemplates = waiverTemplates.filter(
            (t) => t.is_after_payment === true,
          );
          const requiresWaivers = cart.some((item) => {
            if (item.type === "SERVICE")
              return afterPaymentTemplates.some(
                (t) => t.service_id === item.serviceId,
              );
            if (item.type === "MEMBERSHIP")
              return afterPaymentTemplates.some(
          ...
          const finalizeCheckout = async () => {
            ...
            try {
              ...
              setMarketplaceError(
                "Finalization failed. Please refresh the page.",
              );
              setPaymentOpen(false);
            } finally {
              setSubmitting(false);
        ...
        onCreateInvoice={handleCreateInvoiceForCheckout}
      />
      ...
      <ContractsSigningDialog
        ...
        templates={waiverTemplates.filter((t) => {
          if (waiverPhase === "BEFORE_PAYMENT")
            return t.is_before_payment === true;
          if (waiverPhase === "AFTER_INFO_CAPTURED")
            return t.is_after_payment_info_captured === true;
          if (waiverPhase === "AFTER_PAYMENT")
            return t.is_after_payment === true;
          return false;
        })}
        primaryProfile={primaryMember}
        locationId={currentLocationId || ""}
        subscriptionTerms={subscriptionTerms}
        cardLast4={
          waiverPhase === "AFTER_INFO_CAPTURED" ? capturedLast4 : undefined
        }
        mandatoryMode={true}
        basePrices={basePrices}
        membershipPrograms={membershipPrograms}
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" color="text.secondary">
            Are you sure you want to proceed to payment and agreement for the
            selected items? Subscription and invoice will be created during the
            payment step.
          </Typography>
        </DialogContent>
