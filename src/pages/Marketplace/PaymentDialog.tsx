...
interface PaymentDialogProps {
  ...
  onSkip?: () => void;
  onCreateInvoice?: () => Promise<{ invoiceId: string; total: number } | null>;
}
...
export const PaymentDialog = ({
  ...
  const validateForm = () => {
    ...
    if (!expiry.replace(/[^0-9]/g, "").match(/^\d{4}$/))
  ...
  const handlePay = async () => {
    ...
    if (!accountId) {
      setError("Account context missing.");
      return;
    }
    ...
    if (!currentInvoiceId) {
      if (onCreateInvoice) {
        ...
        setError("Invoice context missing and no creation method provided.");
        return;
      }
    }
    ...
    try {
      ...
      if (errorData?.gateway) {
        const gatewayError = errorData.gateway.errorMessage || errorData.gateway.status;
        setError(`Payment Declined by Gateway: ${gatewayError}. Please check your card details and try again.`);
      } else {
        const msg = errorData?.error || errorData?.message || err.message || "Payment processing failed";
        setError(msg);
      }
    } finally {
      setProcessing(false);
        ...
        {success ? (
          <Box sx={{ p: 6, textAlign: "center" }}>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
              Your payment of ${amountToPay.toFixed(2)} was processed
              successfully.
            </Typography>
            <Button
              variant="contained"
                ...
                <Stack spacing={2.5}>
                            ...
                            label={
                              <Box display="flex" alignItems="center" gap={1}>
                                ...
                                <Typography variant="body2" fontWeight={600}>
                                  {card.card_brand || "Card"} ending in{" "}
                                  {card.last4_digits}
                                </Typography>
                                <Typography
                                  variant="caption"
                  ...
                  <Box sx={{ pt: 2 }}>
                    {error && (
                      ...
                      >
                        {processing ? "Retrying..." : "Retry Payment Attempt"}
                      </Button>
                    )}
